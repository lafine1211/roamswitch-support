...
**版** v1.5 · **対象** RoamSwitch 1.8.5 · **要件** macOS 13.0+ / Apple Silicon · **発行** 2026-09-06 · **Team ID** GV76B6G4YU

*v1.5: launchd PathState によるエアギャップ・フェイルセーフの自律再起動機構を追加（特権ヘルパー異常終了時も10分タイムアウトを確実に解除）。*
*v1.4: ポート監査のCookie分離、外部バイナリのパーミッション検証、WireGuard引数サニタイズ、XPC接続の動的コード署名検証（audit_token）による権限昇格対策を反映。*
...
| tailscaleSetKillSwitch(tunnelInterface:active:) tailscaleSetExitNode(_:) | VPN トンネルの Tailscale バックエンド（§5、1.8.0〜）。pf キルスイッチの設置／撤去。および（フォールバック、standalone `tailscaled` 向け——App Store 版 CLI は root で叩かない）`tailscale set --exit-node=<ノード>`／`--exit-node=`（解除）。`exit-node` 引数は英数・`.`・`-`・`:` のみ許可 | `tailscale set`（CLI）／ /sbin/pfctl（`PFRulesetCoordinator` 経由、§4） |
| renewDHCPLease(...) | Tailscale の Exit Node 解除後に routing / DNS を回復（§5）。IPv4 アドレスを持つネットワークサービスを bounce（`networksetup -setnetworkserviceenabled … off/on`）＋ DNS キャッシュフラッシュ。使っていない NIC には触れません | /usr/sbin/networksetup ／ /usr/bin/dscacheutil -flushcache ／ killall -HUP mDNSResponder |
...
1. **コンテンツフィルタ システム拡張（`RoamSwitchLinkFilter`、1.8.0〜、承認後に優先）。**`NEFilterDataProvider` の system extension です。**Apple の申請は不要**——コンテンツフィルタ プロバイダは 2016 年からセルフサービスで、審査キューはありません（監視対象デバイス限定という制約は iOS 側だけの話）。Developer ID 署名 + notarization 済みで、一般の Mac で動きます。初回のみシステム設定での承認が要ります。
   ...
   - `block` 時はフローを `.drop()`。`warn` 時はフローを `.pause()` で保留します（下記）。QUIC（UDP/443）は ClientHello が暗号化されていて SNI を読めないため、`block` モードでは drop してブラウザを HTTP/2（TCP）にフォールバックさせます。
...
- **3 モード。**「オフ」＝無効。「警告のみ」＝該当接続を**一時停止**してユーザーに諮る（下記の実 `warn`）。「明らかな詐欺サイトは自動でブロック（推奨）」＝即遮断。**1.7.2 以降の既定はブロック**。
- **実 `warn`（1.8.0〜、拡張のみ）。**`warn` ヒット時、システム拡張は該当フロー（OS 解決名・SNI の両経路）を `.pause()` で**保留**し、アプリが「許可／ブロック」通知を出します（通知の元は拡張が `/Library/Application Support/RoamSwitch/linkfilter/events.log` に追記する判定行、ホスト単位 1 時間デデュープ）。ユーザーのタップが `allowlist.txt`／`extra.txt` に届いた瞬間——拡張が小さな状態ファイルを 1.5 秒間隔でポーリング——保留フローを resume（許可）または drop（ブロック）します。**25 秒無応答なら fail-open**（`warn` は助言的なので通す）。判断はホスト単位でキャッシュ（許可 5 分／ブロック 1 時間）し、同じページの他フローや再訪は即時・強制。`block` 通知は従来どおり「〜をブロックしました」＋「今回は許可（5 分）」ボタン。
