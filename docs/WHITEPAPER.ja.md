...
**版** v1（初版） · **対象** RoamSwitch 1.5.5 (build 28) · **要件** macOS 13.0+ / Apple Silicon · **発行** 2026-08-30 · **Team ID** GV76B6G4YU
...
### ランサムウェアおとり（カナリア）の自律復元

カナリアファイル（おとり）が改ざん・リネームされて緊急 Air-Gap が発動した場合、安全を確認して隔離を解除すると、破損・消失したカナリアファイルは自動的に正規のベースラインへと再生成・ハッシュ修復され、kqueue リアルタイム監視が途切れることなく即座に再開されます。
