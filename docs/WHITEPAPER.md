...
- **Arming order.** First `tailscale set --exit-node=<node>` runs, then `tailscale status` is polled for up to 12 s until that node is actually the active exit node; only then is the kill-switch installed. If the node never goes active, no kill-switch is installed and you are notified (installing the kill-switch first can sever Tailscale's own control/DERP link and stop `tailscaled`).
