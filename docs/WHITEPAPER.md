...
**Version** v1.5 · **Covers** RoamSwitch 1.8.5 · **Requires** macOS 13.0+ / Apple Silicon · **Published** 2026-09-06 · **Team ID** GV76B6G4YU

*v1.5: adds autonomous air-gap failsafe daemon resurrection via launchd PathState (ensuring the 10-minute packet filter timeout unblocks even if helper crashes).*
...
| tailscaleSetKillSwitch(tunnelInterface:active:) tailscaleSetExitNode(_:) | VPN tunnel's Tailscale backend (§5, 1.8.0+). Install / remove the pf kill-switch; and (fallback, for a standalone `tailscaled` — the App Store CLI is never run as root) `tailscale set --exit-node=<node>` / `--exit-node=` (clear). The `--exit-node` argument is restricted to alphanumerics, `.`, `-`, `:` | `tailscale set` (CLI) / /sbin/pfctl (via `PFRulesetCoordinator`, §4) |
| tailscaleKillSwitchActive(...) | Whether the Tailscale kill-switch (`pf`) is currently loaded | — |
| renewDHCPLease(...) | Recover routing + DNS after a Tailscale exit node is cleared (see §5). Bounces every network *service* that has an IPv4 address (`networksetup -setnetworkserviceenabled … off/on`) and flushes the DNS cache. Never touches an inactive NIC | /usr/sbin/networksetup / /usr/bin/dscacheutil -flushcache / killall -HUP mDNSResponder |
...
| getHelperVersion(...) | Returns the helper's version string (used for app-compatibility checks; currently 1.8.2) | — |
...
- VPN kill-switch (Tailscale) → `block drop all` plus `pass quick` for only: `lo`, Tailscale's `utunN`, CGNAT `100.64.0.0/10` + `fd7a:115c:a1e0::/48`, DNS (`udp`/`tcp` port 53), STUN `udp/3478`, `udp/41641`, DERP `tcp/443`, DHCP, ICMP (wider than WireGuard's — see §5)
...
1. **Content-filter system extension (`RoamSwitchLinkFilter`, 1.8.0+, preferred once approved).** A `NEFilterDataProvider` system extension. **No Apple review** — content-filter providers have been self-serve since 2016, with no approval queue (the supervised-device restriction is iOS-only). Developer ID-signed + notarized, runs on ordinary Macs; the user approves it once in System Settings.
   ...
   - `block` → `.drop()` the flow. `warn` → `.pause()` the flow (see below). QUIC (UDP/443) carries an encrypted ClientHello with no readable SNI, so in `block` mode it is dropped, forcing the browser to fall back to HTTP/2 over TCP.
...
- **Three modes.** "Off" disables it. "Warn only" pauses the matching connection and asks the user (see Real `warn` below). "Auto-block obvious scam sites (recommended)" drops immediately. **The default is block as of 1.7.2.**
- **Real `warn` (1.8.0+, extension only).** On a `warn` hit the system extension holds the flow (both the OS-named and the SNI path) with `.pause()` and the app raises an "Allow / Block" notification (sourced from the verdict line the extension appends to `/Library/Application Support/RoamSwitch/linkfilter/events.log`, one line per host per hour). The moment the user's tap reaches `allowlist.txt` / `extra.txt` — the extension polls the small state files every 1.5 s — the held flows resume (allow) or drop (block). **No answer within 25 s fails open** (a warn is advisory). The decision is cached per host (allow 5 min / block 1 h), so the page's other flows and later visits are instant and enforced. The `block` notification is unchanged: "Blocked …" with an "Allow once (5 min)" button.
...
**(B) Tailscale backend (1.8.0+).** For users who already run Tailscale. RoamSwitch does **not** run `tailscale up` / log in / install `tailscaled` — it reads `tailscale status --json` and runs `tailscale set --exit-node=<node>` / `--exit-node=` (clear).

- **The standalone CLI is recommended** (`brew install tailscale` + `sudo tailscaled install-system-daemon`). RoamSwitch drives it via the helper (root), with `--socket /var/run/tailscaled.socket` set explicitly. **The App Store (GUI) build can't be driven from outside the app** (sandbox) — with it, you pick the exit node in the Tailscale app and RoamSwitch only shows status and manages the kill-switch (running the App Store CLI as root stops the GUI's `tailscaled`, so RoamSwitch never does). The `--exit-node` argument is restricted to alphanumerics, `.`, `-`, `:`.
- **An exit node is required.** Anti-MITM protection is established by choosing an exit node that routes *all* traffic through the tunnel. Without one the feature does not arm. If the chosen exit node goes offline or is unreachable (checked with `tailscale ping`) it is disarmed automatically and you are notified (routing everything through a dead node — black-holing the connection — is worse than "not protected").
- **Network reconfiguration on disconnect.** The standalone macOS `tailscaled` doesn't cleanly restore routing + DNS (the `100.100.100.100` override) when an exit node is cleared — connectivity stays broken until the interface is reset. So when RoamSwitch clears an exit node that was actually active, the helper **bounces the active network service(s)** (`networksetup -setnetworkserviceenabled … off/on` — the same as toggling Wi-Fi by hand; only services with an IPv4 address, safe for multi-NIC and static-IP setups). Traffic drops for ~5–10 s and RoamSwitch does not demote the protection level during that window.
- **Arming order.** First the exit node is set and `tailscale status` polled for up to 12 s until it is active; only then is the (opt-in) kill-switch installed — installing it first can sever Tailscale's own control/DERP link and stop `tailscaled`.
- **The kill-switch is off by default (opt-in).** The exit node already tunnels everything; the pf `block drop all` only adds "no plaintext leaks even if Tailscale glitches" (and on some setups can knock out Tailscale's own connection, hence opt-in).
- **Kill-switch (`pf`, `block drop all`).** Looser than the WireGuard one: Tailscale's transport can't be pinned to one endpoint IP (direct UDP to roaming peers + DERP relays over TCP 443 / UDP 3478). It permits: the Tailscale tun (`utunN` — dynamic on macOS, so at arm time the interface holding a `100.64.0.0/10` address is found via `ifconfig`), the CGNAT ranges `100.64.0.0/10` and `fd7a:115c:a1e0::/48`, **DNS (`udp`/`tcp` port 53 — `tailscaled` stops if it can't resolve control/DERP hostnames)**, STUN `udp/3478`, Tailscale direct `udp/41641`, DERP `tcp/443`, DHCP, ICMP. **Everything else (SMB, mDNS, plaintext HTTP, arbitrary TCP) is dropped.** Allowing DNS makes this "leak-resistant", not "leak-proof" like the WireGuard kill-switch.
