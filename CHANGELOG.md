...
- **Fixed: with the network isolated by an ARP spoof, the check that the cause is gone could never succeed.** On a real Mac the gateway's entry vanished from the ARP table within seconds of the isolation and never came back (17 minutes 34 seconds until it was released by hand), and the check only reads the table. While the entry is missing, the helper now sends one ARP request (link level, not IP, so the isolation does not drop it) for the default gateway, at most once every 10 seconds, so what really answers is checked again. The 3-checks-over-60-seconds condition is unchanged; a forged answer still counts as a mismatch. The frame is sent through BPF; checked on a real Mac.
